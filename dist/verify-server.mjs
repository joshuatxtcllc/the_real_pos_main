...
async function getCachedPricing(frameId, matColorId, glassOptionId, dimensions) {
  const cacheKey = `pricing:${frameId}:${matColorId}:${glassOptionId}:${dimensions.width}x${dimensions.height}x${dimensions.matWidth}`;
  return await cache.get(cacheKey);
}
async function setCachedPricing(frameId, matColorId, glassOptionId, dimensions, pricing) {
  const cacheKey = `pricing:${frameId}:${matColorId}:${glassOptionId}:${dimensions.width}x${dimensions.height}x${dimensions.matWidth}`;
  await cache.set(cacheKey, pricing, 6e5);
}
...
var logger, StructuredLogger, structuredLogger;
  ...
  "server/utils/logger.ts"() {
    ...
    StructuredLogger = class {
      ...
      warn(message, context) {
        this.winston.warn(message, { ...context, timestamp: (/* @__PURE__ */ new Date()).toISOString() });
      }
      error(message, errorLog) {
        ...
        if (errorLog.severity === "critical" && true) {
          this.sendToCriticalMonitoring(errorData);
        }
      }
      integration(integration, operation, success, context) {
        const logData = {
          integration,
          operation,
          success,
...
var init_circuitBreaker = __esm({
  "server/middleware/circuitBreaker.ts"() {
    ...
    circuitBreakers = {
      database: new CircuitBreaker("Database", 3, 3e4),
      openai: new CircuitBreaker("OpenAI", 5, 6e4),
      stripe: new CircuitBreaker("Stripe", 3, 45e3),
      supabase: new CircuitBreaker("Supabase", 4, 3e4),
      larsonApi: new CircuitBreaker("Larson API", 5, 12e4),
      twilio: new CircuitBreaker("Twilio", 3, 3e4)
    };
  }
});
...
function calculateFallbackPricing(params) {
  ...
  const framePrice = unitedInches * 2.5;
  const matPrice = unitedInches * 1.8;
  const glassPrice = unitedInches * 1.2;
  const backingPrice = 15;
  const laborCost = Math.max(unitedInches * 0.8, 25);
...
async function calculatePricingWithFallbacks(params) {
  const {
    frameId,
    matColorId,
    glassOptionId,
