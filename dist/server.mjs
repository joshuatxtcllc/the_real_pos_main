...
var init_storage = __esm({
  "server/storage.ts"() {
    ...
    DatabaseStorage = class {
      ...
      async updateMaterialOrder(materialId, updateData) {
        try {
          const materials = await this.getMaterialsPickList();
...
async function recordStatusChange(orderId, previousStatus, newStatus, notes) {
  try {
    ...
    `);
    const [record] = await db.execute(sql3`
...
async function getOrderStatusHistory(orderId) {
  try {
    const history = await db.execute(sql3`
      SELECT * FROM order_status_history 
      WHERE order_id = ${orderId}
...
async function notifyCustomerOfStatusChange(order, previousStatus, newStatus) {
  try {
    ...
    const [customer] = await db.execute(sql3`
...
async function getOrderWithQrCode(req, res) {
  try {
    ...
    const qrCodeData = await (void 0)(orderId);
    res.json({
      success: true,
      order: {
        ...order,
...
async function getAllOrdersWithQrCodes(req, res) {
  try {
    ...
    const enhancedOrders = await Promise.all(limitedOrders.map(async (order) => {
      const qrCodeData = await (void 0)(order.id);
      return {
        ...order,
        qrCode: qrCodeData
      };
...
\u2022 \`/help\` - Show this help message
  ...
  async setupCommands() {
    const commands = [
      new SlashCommandBuilder().setName("order-status").setDescription("Check the status of a framing order").addStringOption(
        (option) => option.setName("order-id").setDescription("The order ID to check").setRequired(true)
      ),
      new SlashCommandBuilder().setName("frame-quote").setDescription("Get a quick frame quote").addStringOption(
        (option) => option.setName("dimensions").setDescription("Frame dimensions (e.g., 16x20)").setRequired(true)
      ),
      new SlashCommandBuilder().setName("production-status").setDescription("Check production queue status"),
      new SlashCommandBuilder().setName("help").setDescription("Show available commands and help"),
      new SlashCommandBuilder().setName("inventory-status").setDescription("Check current inventory levels").addStringOption(
        (option) => option.setName("item").setDescription("Specific item to check (optional)").setRequired(false)
      )
  ...
  async sendOrderUpdate(channelId, orderId, status) {
    const message = `\u{1F514} **Order Update**
Order #${orderId} status changed to: **${status}**`;
    await this.sendNotification(channelId, message);
  }
  async sendProductionAlert(channelId, message) {
...
${message}`;
  ...
  async sendOrderStatusDM(userId, orderId, status, details) {
    const message = `\u{1F514} **Order Update - Jays Frames**
...
Thank you for choosing Jays Frames!`;
  ...
  async sendCompletionNoticeDM(userId, orderId, pickupInfo) {
...
We can't wait for you to see your beautiful custom frame!`;
  ...
  async sendEstimateUpdateDM(userId, orderId, estimatedDays) {
    const message = `\u{1F4C5} **Timeline Update - Jays Frames**
...
We'll keep you posted on any changes!`;
  ...
  async sendCustomNotificationDM(userId, title, message) {
...
var discordBot_default = DiscordBot;
...
var NotificationController = class {
  ...
  sendNotification(notification, targetApps) {
    ...
    this.connectedClients.forEach((client) => {
      if (!targetApps || targetApps.includes(client.appId)) {
        if (client.socket.readyState === ws2.OPEN) {
...
// server/services/customerNotificationService.ts
init_db();
import { sql as sql2 } from "drizzle-orm";
async function getCustomerNotifications(customerId, limit = 50) {
  try {
    const notifications2 = await db.execute(sql2`
      SELECT 
        n.*,
        o.id as order_id,
        o.frame_name,
...
async function createCustomerNotification(customerId, orderId, type, message) {
  try {
    const notification = await db.execute(sql2`
...
async function markNotificationsAsRead(notificationIds) {
  try {
    await db.execute(sql2`
      ...
      SET read = true 
      WHERE id IN (${sql2.join(notificationIds)})
    `);
    return true;
  } catch (error) {
...
var UnifiedNotificationService = class {
   ...
   */
  async sendDiscordNotification(userId, notification) {
    try {
   ...
   */
  async sendEmailNotification(customer, notification) {
    try {
      ...
      notificationController.sendNotification({
        ...
        link: notification.orderId ? `/orders/${notification.orderId}` : void 0,
        smsEnabled: false,
   ...
   */
  async sendSmsNotification(customer, notification) {
    try {
   ...
   */
  async sendInAppNotification(customer, notification) {
    try {
      notificationController.sendNotification({
        id: Date.now(),
        title: notification.title,
   ...
   */
  extractDaysFromMessage(message) {
   ...
   */
  async sendOrderStatusUpdate(customer, orderId, status, details) {
   ...
   */
  async sendCompletionNotice(customer, orderId, pickupInfo) {
    await this.notifyCustomer(customer, {
   ...
   */
  async sendEstimateUpdate(customer, orderId, estimatedDays) {
    await this.notifyCustomer(customer, {
...
var unifiedNotificationService_default = UnifiedNotificationService;
...
// server/routes/discordNotificationRoutes.ts
import express from "express";
var router8 = express.Router();
router8.post("/test-discord-notification", async (req, res) => {
  try {
    const { discordUserId, orderId, type, message } = req.body;
    if (!discordUserId) {
      return res.status(400).json({ error: "Discord user ID is required" });
    }
    const notificationService2 = req.app.locals.notificationService;
    const testCustomer = {
      id: 1,
    ...
    switch (type) {
      case "order_update":
        result = await notificationService2.sendOrderStatusUpdate(
          testCustomer,
          orderId || 123,
          "In Production",
        ...
        break;
      case "completion":
        result = await notificationService2.sendCompletionNotice(
          testCustomer,
          orderId || 123,
        ...
        break;
      case "estimate":
        result = await notificationService2.sendEstimateUpdate(
          testCustomer,
          orderId || 123,
          7
        );
        break;
      default:
        result = await notificationService2.notifyCustomer(testCustomer, {
...
var discordNotificationRoutes_default = router8;

// server/routes/customerNotificationRoutes.ts
import express2 from "express";
var router9 = express2.Router();
router9.get("/customers/:customerId/notifications", async (req, res) => {
  try {
    ...
    const notifications2 = await getCustomerNotifications(customerId, limit);
...
router9.post("/notifications/mark-read", async (req, res) => {
  try {
...
var customerNotificationRoutes_default = router9;

// server/routes/testNotificationRoutes.ts
import express3 from "express";
var router10 = express3.Router();
router10.post("/test-customer-notification", async (req, res) => {
  try {
    const {
      customerId = 1,
      orderId = 123,
    ...
    const discordBot2 = req.app.locals.discordBot;
    if (!discordBot2) {
      return res.status(500).json({ error: "Discord bot not initialized" });
    }
    const notificationService2 = new unifiedNotificationService_default(discordBot2);
    const testCustomer = {
      id: customerId,
      email,
      phone,
...
router10.post("/test-all-channels", async (req, res) => {
  try {
    ...
    const tests = [
      {
        type: "order_update",
        test: () => notificationService2.sendOrderStatusUpdate(testCustomer, 456, "Measuring", "We are carefully measuring your artwork.")
      },
      {
        type: "completion",
        test: () => notificationService2.sendCompletionNotice(testCustomer, 456, "Your frame is ready! Please bring your pickup receipt.")
      },
      {
        type: "estimate",
        test: () => notificationService2.sendEstimateUpdate(testCustomer, 456, 5)
      }
    ];
    const results = [];
...
var testNotificationRoutes_default = router10;
...
async function registerRoutes(app2) {
  ...
  app2.post("/api/notifications/send", async (req, res) => {
    try {
      ...
      if (discordUserId && !customer.discordUserId) {
        updateCustomerDiscordId2(customer.id, discordUserId);
        customer.discordUserId = discordUserId;
      }
      const notificationService2 = req.app.locals.notificationService;
      let result;
      ...
      switch (type) {
        ...
        case "estimate":
          ...
          break;
        default:
          result = await notificationService2.notifyCustomer(customer, {
  ...
  app2.use("/api/discord", discordNotificationRoutes_default);
  app2.use("/api/notifications", customerNotificationRoutes_default);
  app2.use("/api/test", testNotificationRoutes_default);
...
function serveStatic(app2) {
  ...
  if (staticPath) {
    app2.use(express4.static(staticPath, {
      maxAge: true ? "1d" : 0,
      etag: true,
      index: "index.html"
    }));
...
try {
  discordBot = new discordBot_default();
  notificationService = new unifiedNotificationService_default(discordBot);
  discordBot.start();
} catch (error) {
  log2(`Warning: Discord bot initialization failed: ${error}`, "warning");
